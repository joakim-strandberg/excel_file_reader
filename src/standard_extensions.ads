...
package Standard_Extensions is
   ...
   subtype Subpool_Handle is Dynamic_Pools.Subpool_Handle;

   Empty_String : aliased String := "";

   type String_Ptr is access all Standard.String with
     Storage_Pool => Default_Subpool;

   type CString is new String with
     Dynamic_Predicate =>
       (CString'Length > 0 and then
          CString (CString'Last) = Character'Val (0));

   function "+" (Text : String) return CString;

   function "-" (Text : CString) return String;
   ...
   function To_Hash32 (This : Int32) return Standard_Extensions.Hash32 with
     Global => null;
   ...
   function I (Source : String;
     ...
     (Standard_Extensions.To_Int32 (Source (Source'First + Index))) with
     Pre   =>
       (Index < Source'Length and then
          Source'First + Index <= Source'Last and then
            Standard_Extensions.Is_Digit (Source (Source'First + Index)));
        ...
        Source'Length = 1 =>
          (if Source (Source'First) = '-' then
               Has_Failed
                   ...
                   Standard_Extensions.Is_Digit (Source (Source'First)) then
     ...
     Pre =>
       ((Source'Length >= 1 and Source'Length <= 11) and then (
              if Source'Length = 1 then
                Standard_Extensions.Is_Digit (Source (Source'First))
   ...
   function Ends_With (This         : String;
                       Searched_For : String) return Boolean with
     Global => null,
     Pre    => Searched_For'Length > 0;
   ...
   function Count_Characters
     (Source       : String;
      Searched_For : Character) return Nat32;
   --  Counts the number of occurenses of the Searched_For character
   ...
   type Subprogram_Call_Result is tagged limited private with
     Default_Initial_Condition => Subprogram_Call_Result.Has_Failed = False;

   procedure Initialize (This   : in out Subprogram_Call_Result;
                         Code_1 : Int32;
                         Code_2 : Int32) with
     Global     => null,
   ...
   function Is_Success (This : Subprogram_Call_Result) return Boolean with
     Global => null;

   function Has_Failed (This : Subprogram_Call_Result) return Boolean with
     Global => null;

   function Message (This : Subprogram_Call_Result) return String with
     Global => null;
...
private
   ...
   type Subprogram_Call_Result is tagged limited
      record
         My_Code_1 : Int32 := 0;
         My_Code_2 : Int32 := 0;
   ...
   function Has_Failed (This : Subprogram_Call_Result) return Boolean is
     (This.My_Has_Failed);

   function Is_Success (This : Subprogram_Call_Result) return Boolean is
      (not This.My_Has_Failed);
